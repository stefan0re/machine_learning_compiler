...
void run_example_with_einsum(float* i_ten_1,
    ...
    std::vector<TensorOperation::exec_t> l_exec_types = {TensorOperation::exec_t::seq,
                                                         ...
                                                         br_gemm ? TensorOperation::exec_t::prim : TensorOperation::exec_t::seq,
...
int main() {
    ...
    float* l_out_einsum_2 = new float[32 * 32 * 32 * 32];
    float* l_out_einsum_3 = new float[32 * 32 * 32 * 32];
    ...
    for (size_t i = 0; i < 32 * 32 * 32 * 32; ++i) {
        ...
        l_out_einsum_2[i] = 0.0f;
        l_out_einsum_3[i] = 0.0f;
    }
    ...
    run_example_with_einsum(l_ten_1, l_ten_2, l_out_einsum_1, false, false, false);

    bool equal = check_diff(l_out_scalar, l_out_einsum_1, 32 * 32 * 32 * 32);

    if (equal) {
        std::cout << "Example 1: Scalar and Einsum results are equal." << std::endl;
    } else {
        std::cout << "Example 1: Scalar and Einsum results differ!" << std::endl;
    }

    std::cout << "*******************************************************" << std::endl;

    std::cout << "Testing Example 2" << std::endl;
    run_example_with_einsum(l_ten_1, l_ten_2, l_out_einsum_2, true, false, false);

    equal = check_diff(l_out_scalar, l_out_einsum_2, 32 * 32 * 32 * 32);

    if (equal) {
        std::cout << "Example 2: Scalar and Einsum results are equal." << std::endl;
    } else {
        std::cout << "Example 2: Scalar and Einsum results differ!" << std::endl;
    }

    std::cout << "*******************************************************" << std::endl;

    std::cout << "Testing Example 3 with ReLU and first touch zero" << std::endl;
    run_example_with_einsum(l_ten_1, l_ten_2, l_out_einsum_3, true, true, true);

    for (size_t i = 0; i < 32 * 32 * 32 * 32; ++i) {
        l_out_scalar[i] = std::max(0.0f, l_out_scalar[i]);
    }

    equal = check_diff(l_out_scalar, l_out_einsum_3, 32 * 32 * 32 * 32);

    if (equal) {
        std::cout << "Example 3: Scalar and Einsum results are equal." << std::endl;
    } else {
        std::cout << "Example 3: Scalar and Einsum results differ!" << std::endl;
    }
